...
const EN_ABOUT: About = About {
    ...
    no_api: "**No Telegram bot, token, or API is needed** — the site is built from the public web preview, with all media except audio and big videos downloaded locally, so it keeps working even if the channel is removed.",
    kind_text: "Text",
    kind_images: "Images",
    kind_videos: "Videos",
    kind_audio: "Audio",
...
const RU_ABOUT: About = About {
    ...
    no_api: "**Не нужны ни бот, ни токен, ни API Telegram** — сайт собран из публичного веб-превью, все медиа, кроме аудио и больших видео, скачаны локально, поэтому он продолжит работать, даже если канал удалят.",
    kind_text: "Текст",
...
const UK_ABOUT: About = About {
    ...
    no_api: "**Не потрібні ні бот, ні токен, ні API Telegram** — сайт зібрано з публічного вебпрев’ю, усі медіа, крім аудіо та великих відео, завантажено локально, тож він працюватиме, навіть якщо канал видалять.",
    kind_text: "Текст",
...
const BE_ABOUT: About = About {
    ...
    no_api: "**Не патрэбныя ні бот, ні токен, ні API Telegram** — сайт сабраны з публічнага вэб-прэв’ю, усе медыя, апроч аўдыя і вялікіх відэа, спампаваныя лакальна, таму ён будзе працаваць, нават калі канал выдаляць.",
    kind_text: "Тэкст",
    kind_images: "Выявы",
    kind_videos: "Відэа",
    kind_audio: "Аўдыя",
...
const DE_ABOUT: About = About {
    ...
    no_api: "**Kein Telegram-Bot, -Token oder -API nötig** — die Website wird aus der öffentlichen Web-Vorschau erstellt, alle Medien außer Audio und großen Videos werden lokal gespeichert, sodass sie auch dann funktioniert, wenn der Kanal entfernt wird.",
    kind_text: "Text",
    kind_images: "Bilder",
    kind_videos: "Videos",
    kind_audio: "Audio",
...
const FR_ABOUT: About = About {
    ...
    no_api: "**Aucun bot, jeton ni API Telegram n’est nécessaire** — le site est généré à partir de l’aperçu web public, et tous les médias sauf l’audio et les vidéos volumineuses sont téléchargés localement, de sorte qu’il continue de fonctionner même si la chaîne est supprimée.",
    kind_text: "Texte",
    kind_images: "Images",
    kind_videos: "Vidéos",
    kind_audio: "Audio",
...
const ZH_ABOUT: About = About {
    ...
    source_repo: "源代码仓库：",
    no_api: "**无需 Telegram 机器人、令牌或 API** — 本站由公开网页预览生成，除音频和大视频外的所有媒体均已下载到本地，因此即使频道被删除也能继续访问。",
    kind_text: "文本",
    kind_images: "图片",
    kind_videos: "视频",
    kind_audio: "音频",
...
const JA_ABOUT: About = About {
    ...
    no_api: "**Telegram のボット・トークン・API は不要** — このサイトは公開ウェブプレビューから生成され、音声と大きな動画を除くすべてのメディアはローカルに保存されるため、チャンネルが削除されても表示できます。",
    kind_text: "テキスト",
    kind_images: "画像",
    kind_videos: "動画",
    kind_audio: "音声",
...
const PL_ABOUT: About = About {
    ...
    no_api: "**Nie potrzeba bota, tokena ani API Telegrama** — witryna jest budowana z publicznego podglądu, a wszystkie media oprócz audio i dużych filmów są pobierane lokalnie, więc działa nawet po usunięciu kanału.",
    kind_text: "Tekst",
    kind_images: "Obrazy",
    kind_videos: "Wideo",
    kind_audio: "Audio",
...
const ES_ABOUT: About = About {
    ...
    no_api: "**No se necesita ningún bot, token ni API de Telegram** — el sitio se genera a partir de la vista previa pública y todos los archivos excepto el audio y los vídeos grandes se descargan localmente, así que sigue funcionando aunque se elimine el canal.",
    kind_text: "Texto",
...
const KO_ABOUT: About = About {
    ...
    no_api: "**Telegram 봇, 토큰, API가 필요 없습니다** — 이 사이트는 공개 웹 미리보기로 생성되며 오디오와 큰 동영상을 제외한 모든 미디어가 로컬에 저장되므로 채널이 삭제되어도 계속 볼 수 있습니다.",
    kind_text: "텍스트",
    kind_images: "이미지",
    kind_videos: "동영상",
    kind_audio: "오디오",
...
const KA_ABOUT: About = About {
    ...
    no_api: "**Telegram-ის ბოტი, ტოკენი ან API საჭირო არ არის** — საიტი იქმნება საჯარო ვებ-გადახედვიდან, ყველა მედია, აუდიოსა და დიდი ვიდეოების გარდა, ჩამოტვირთულია ლოკალურად, ამიტომ ის იმუშავებს მაშინაც კი, თუ არხი წაიშლება.",
    kind_text: "ტექსტი",
...
const HI_ABOUT: About = About {
    ...
    no_api: "**किसी Telegram बॉट, टोकन या API की आवश्यकता नहीं** — साइट सार्वजनिक वेब पूर्वावलोकन से बनाई जाती है और ऑडियो और बड़े वीडियो को छोड़कर सभी मीडिया स्थानीय रूप से डाउनलोड किए जाते हैं, इसलिए चैनल हटाए जाने पर भी यह काम करती रहती है।",
    kind_text: "टेक्स्ट",
    kind_images: "छवियाँ",
    kind_videos: "वीडियो",
    kind_audio: "ऑडियो",
