...
| `OFFLINE` | `--offline` | off | `true` for offline mode + an installable **PWA**: a service worker precaches the whole archive (audio, video, attachments) on the first visit over **any non-cellular connection** (Wi-Fi or wired — it skips only cellular / Data Saver), and `display: standalone` makes it installable (hides the address bar). Needs JavaScript; built-in templates only |
