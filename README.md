...
| `RELATED` | `--related` | off | `true` appends a **"Related" list** to each post — the posts that share the most **tags** with it (the whole tag set, not just one tag), linked internally. Ranked by shared-tag count; up to 5. Posts with no shared-tag neighbour get none |
